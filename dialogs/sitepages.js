CKEDITOR.dialog.add( 'sitepagesDialog', function( editor ) {
    return {
                ...
                elements: [
                    {
                        ...
                        elemId : null, 
                        items : [ ['--- Select a page---',0] ],
                        
                        onLoad: function(element) {
                        	var element_id = '#' + this.getInputElement().$.id;
							
							$.getJSON('/admin/pages/pages.json', function(jsondata){
								var html = '';
								var len = jsondata.length;
								for (var i = 0; i< len; i++) {
									html += '<option value="/' + jsondata[i].value + '">' + jsondata[i].option + '</option>';
								}
        ...
        onOk: function() {
            ...
            link.setAttribute( 'href', dialog.getValueOf( 'tab-basic', 'sitepages' ) );
